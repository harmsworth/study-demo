...
# 10、`HTTP` 的内容编码
  1. 服务器在发送内容实体时，会压缩实体信息，客户端接收并负责解码。
  2. 常见内容编码有 `gzip`、`compress` 等
  3. 服务器在发送大容量数据时，会将数据分割成很多块，客户端逐步显示页面。
...
# 14、`HTTP` 状态码
  ...
  - 2XX 成功
    ...
    - 206 Partial Content 客户端进行范围请求，服务器成功响应。响应报文 `Content-Range` 指定范围的实体内容。
  ...
  - 5XX 服务器错误
    - 500 Internal Server Error 服务器执行请求出现错误
...
# 16、HTTPS 缺点
  ...
  - `HTTPS` 需要对客户端和服务器进行加密、解密处理，所以会消耗更多的硬件资源
